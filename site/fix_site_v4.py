...
def process_file(filepath, prefix=""):
    try:
        ...
        # If in projects/, likely Portfolio
        ...
        # Clean up script tags that might be duplicates or old
        ...
        # Standardize project logos size (if it has logo-anim-wrap)
        if "logo-anim-wrap" in content:
            # Remove inline style on logo-anim-wrap
            ...
            def add_logo_anim_class(m):
                attrs = m.group(1)
                if 'logo-anim' not in attrs:
                    # Find class="..." or add it
                    if 'class="' in attrs:
                        return f'<img {attrs.replace(\'class="\', \'class="logo-anim \')}>'
                    else:
                        return f'<img class="logo-anim" {attrs}>'
                return m.group(0)
            
            content = re.sub(r'<img\s+([^>]*alt="[^"]*Logo"[^>]*)>', add_logo_anim_class, content, flags=re.IGNORECASE)

        # Global repairs
        content = content.replace("2025 Navonmesh", "2026 Navonmesh")
        content = content.replace("© 2025", "© 2026")
        
        # Fix relative assets (if src starts with assets/ or css/ or js/ but should have prefix)
        if prefix:
...
# Blog specific files inside projects
